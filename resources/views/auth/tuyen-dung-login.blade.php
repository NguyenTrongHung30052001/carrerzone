<x-employers-guest-layout>
    ...
    <div class="mb-8 border-b border-gray-300">
        <div class="flex justify-center space-x-8 -mb-px">
            ...
            <a href="{{ route('employers.login') }}" class="py-2 px-4 text-lg font-semibold border-b-2 border-indigo-600 text-indigo-600">
                Đăng nhập
            </a>
            {{-- Tab Đăng ký (Inactive) --}}
            <a href="{{ route('employers.register') }}" class="py-2 px-4 text-lg font-semibold text-gray-500 border-b-2 border-transparent hover:text-indigo-600 hover:border-indigo-600 transition duration-150">
                Đăng ký
            </a>
        </div>
    </div>

    <form method="POST" action="{{ route('employers.login') }}" class="space-y-4">
        @csrf
        ...
        <div class="flex justify-between items-center mt-4">
            ...
            {{-- (Bạn có thể thêm link quên mật khẩu sau) --}}
        </div>
